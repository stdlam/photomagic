...
open class BaseColorPicker(context: Context, attributeSet: AttributeSet?) :
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        event?.let { e ->
            ...
            return when (e.action and e.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    Log.d("PickerMoving", "onTouchEvent ACTION_DOWN (${e.x}, ${e.y})")
                    changePositionOfCircle(e.x, e.y)
                    true
                }
                MotionEvent.ACTION_MOVE -> {
                    Log.d("PickerMoving", "onTouchEvent ACTION_MOVE (${e.x}, ${e.y})")
                    changePositionOfCircle(e.x, e.y)
                    true
                }
