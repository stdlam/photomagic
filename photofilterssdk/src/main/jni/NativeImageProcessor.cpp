...
void saturation(int *pixels, float levelL, float levelS, float levelH, int width, int height, int baseColor) {
    unsigned int r = (unsigned int) (baseColor >> 16) & 0xFF;
    unsigned int g = (unsigned int) (baseColor >> 8) & 0xFF;
    unsigned int b = (unsigned int) (baseColor) & 0xFF;
    ...
    float L = levelL, S = levelS, H = levelH;
...
void saturation2(int *pixels, float level, int width, int height) {
    unsigned int r;
    unsigned int g;
    unsigned int b;
...
Java_com_zomato_photofilters_imageprocessors_NativeImageProcessor_doSaturation(JNIEnv *env, jobject thiz,
                                                                               jintArray pixels, jfloat levelL, jfloat levelS, jfloat levelH,
                                                                               jint width, jint height, jint baseColor) {
    jint *pixelsBuff = getPointerArray(env, pixels);
    saturation(pixelsBuff, levelL, levelS, levelH, width, height, baseColor);
    jintArray result = jintToJintArray(env, width * height, pixelsBuff);
    releaseArray(env, pixels, pixelsBuff);
    return result;
}
